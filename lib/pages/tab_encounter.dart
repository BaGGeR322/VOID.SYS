...
class TabEncounter extends StatelessComponent {
  ...
  Component _buildDaemonList() {
    return div(classes: 'tab-encounter', [
      ...
      div(classes: 'enc-subtitle', [
        .text(
            'Corrupted memory fragments. They do not know they were human once.'),
      ]),
  ...
  Component _buildDaemonEntry(Encounter enc) {
    ...
    if (!isDefeated && !isAvailable) {
      if (enc.requiredUpgrade != null &&
          !state.purchasedUpgrades.contains(enc.requiredUpgrade!)) {
        lockReason = 'requires: ${enc.requiredUpgrade}';
      } else if (enc.requiredDefeated != null &&
          !state.defeatedEncounters.contains(enc.requiredDefeated!)) {
    ...
    return div(
      classes: isDefeated
          ...
          : (isAvailable
              ? 'daemon-entry daemon-entry--available'
              : 'daemon-entry daemon-entry--locked'),
      [
        ...
        div(classes: 'daemon-lore', [.text(enc.lore)]),
  ...
  Component _buildCombat() {
    ...
    final lastLog = state.combatLog.length > 8
        ? state.combatLog.sublist(state.combatLog.length - 8)
        : state.combatLog;
    ...
    return div(classes: 'combat-screen', [
      div(classes: 'combat-header', [
        ...
        if (state.encounterPhase == 2)
          span(classes: 'phase-indicator', [.text('  [ PHASE 2 ]')]),
      ]),
      div(classes: 'combat-arena', [
        div(classes: 'combat-daemon-side', [
          ...
          if (state.enemyStunned)
            div(classes: 'stun-indicator', [.text('[ STUNNED ]')]),
        ]),
        div(classes: 'combat-player-side', [
          ...
          div(classes: 'combat-moves', [
            ...
            for (final move in GameData.playerMoves)
              _buildMoveButton(move),
          ]),
          TermButton(
            label: 'RETREAT',
      ...
      div(classes: 'combat-log', [
        ...
        div(classes: 'combat-log-entries', [
          for (final entry in lastLog)
            div(classes: 'combat-log-entry', [.text(entry)]),
        ]),
      ]),
    ]);
  }
